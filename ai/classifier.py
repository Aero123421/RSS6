...
class Classifier:
    ...
    def __init__(self, api, prompt_template: Optional[str] = None):
        """
        初期化
    ...
    async def classify(
        self, title: str, content: str, categories: List[str] = None
    ) -> str:
        """
        記事のジャンルを分類する
        ...
        try:
            ...
            if self.prompt_template:
                prompt = self.prompt_template.format(
                    categories=categories_str, content=classification_text
                )
            else:
                prompt = f"""
次の記事のジャンルを以下のカテゴリから最も適切なもの一つだけ選んでください:{categories_str}
...
"""
            ...
            # APIを使用して分類
            result = await self.api.generate_text(
