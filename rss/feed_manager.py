...
class FeedManager:
    ...
    def __init__(self, config: Dict[str, Any], ai_processor: Any, discord_bot: Any):
        ...
        self.article_queue: asyncio.Queue[Tuple[Dict[str, Any], Dict[str, Any]]] = (
            asyncio.Queue()
        )
        ...
        try:
            ...
            for feed in feeds:
                try:
                    ...
                    logger.error(
                        f"フィード確認中にエラーが発生しました: {feed.get('url')}: {e}",
                        exc_info=True,
                    )
            ...
            logger.error(
                f"フィード確認中に予期しないエラーが発生しました: {e}", exc_info=True
            )
    ...
    async def _get_new_articles(
        self, feed_data: Dict[str, Any], feed_info: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        新しい記事を取得する
    ...
    async def add_feed(
        ...
        try:
            ...
            if not feed_data:
                return (
                    False,
                    "フィードの解析に失敗しました。有効なRSS/atomフィードであることを確認してください。",
                    None,
                )
            ...
            logger.error(
                f"フィード追加中にエラーが発生しました: {url}: {e}", exc_info=True
            )
            return False, f"エラーが発生しました: {str(e)}", None

    async def remove_feed(
        self, url: str, notify_channel: bool = True
    ) -> Tuple[bool, str]:
        """
        フィードを削除する
        ...
        try:
            ...
            for i, feed in enumerate(feeds):
                if feed.get("url") == url:
                    ...
                    return (
                        True,
                        f"フィード「{removed_feed.get('title', url)}」を削除しました",
                    )
            ...
            logger.error(
                f"フィード削除中にエラーが発生しました: {url}: {e}", exc_info=True
            )
