...
class DiscordBot:
    ...
    def _setup_event_handlers(self):
        ...
        async def on_ready():
            """ボット起動時のイベントハンドラ"""
            logger.info(
                f"Discordボットとしてログインしました: {self.bot.user.name} ({self.bot.user.id})"
            )

            # スラッシュコマンドの登録
            await self.bot.add_cog(RSSCog(self.bot))

            # ステータスの設定
            await self.bot.change_presence(
                activity=discord.Activity(
                    type=discord.ActivityType.watching, name="RSS Feeds"
                )
            )
        ...
        async def on_guild_join(guild):
            ...
            for admin_id in admin_ids:
                try:
                    ...
                    logger.error(
                        f"管理者通知中にエラーが発生しました: {e}", exc_info=True
                    )
        ...
        async def on_message(message):
            ...
            if ref and ref.author.id == self.bot.user.id:
                article = await self.feed_manager.article_store.get_full_article(
                    str(ref.id)
                )
                if article:
                    keywords = await self.ai_processor._generate_search_keywords(
                        article, message.content
                    )
                    related = (
                        await self.feed_manager.article_store.find_related_articles(
                            keywords, str(ref.id)
                        )
                    )
                    answer = await self.ai_processor.answer_question(
                        article, related, message.content
                    )
        ...
        async def on_guild_channel_delete(channel):
            ...
            logger.info(
                f"チャンネル削除に伴い {len(targets)} 件のフィードを削除しました: {channel_id}"
            )
    ...
    async def post_article(
        self, article: Dict[str, Any], channel_id: str
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            logger.info(
                f"記事を投稿しました: {article.get('title')} -> #{channel.name}"
            )
            ...
            logger.error(
                f"記事投稿中にエラーが発生しました: {article.get('title')}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def create_feed_channel(
        ...
        try:
            ...
            logger.info(
                f"フィードチャンネルを作成しました: #{channel.name} ({channel.id})"
            )
            ...
            embed.add_field(
                name="更新頻度",
                value=f"{self.config.get('check_interval', 15)}分ごと",
                inline=True,
            )
            ...
            logger.error(
                f"チャンネル作成中にエラーが発生しました: {feed_info.get('title')}: {e}",
                exc_info=True,
            )
            return None
